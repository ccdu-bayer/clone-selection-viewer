...
local({
  ...
  renv_bootstrap_download <- function(version) {
    ...
    # be a development version; check for the package on GitHub
  ...
  renv_bootstrap_validate_version <- function(version, description = NULL) {
    ...
    dev <- identical(renv_bootstrap_load_type(), "development")
    remote <- if (dev)
  ...
  renv_bootstrap_paths_renv <- function(..., project = NULL) {
    renv <- Sys.getenv("RENV_PATHS_RENV", unset = "renv")
    file.path(project %||% getwd(), renv, ...)
  }
  ...
  renv_json_read_default <- function(file = NULL, text = NULL) {
    ...
    if (!identical(c(locs), -1L)) {
      ...
      stringsp <- gsub('\\."', '\\.', strings, perl = TRUE)
      strings <- strings[stringsp != strings]
